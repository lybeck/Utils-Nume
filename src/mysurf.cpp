...
namespace myplot {
    ...
    static const std::string plot_dir = "data";
    ...
    static bool exists_dir(std::string const& dir) {
        struct stat sb;
    ...
    void surf(surf_data_set data, const char* title, int sample_size, int iso_sample_size) {
        ...
        if (!exists_dir(plot_dir)) {
            int status = mkdir(plot_dir.c_str(), 0777);
            if (status != 0) {
                std::cerr << "Could not create directory '" << plot_dir << "'. Cannot display plot." << std::endl;
                return;
            }
        }
